...
- Dependentes são registrados como PatientAccess, não como Person (permitem que dependentes menores ou incapazes de gerenciar própria conta sejam registrados sem dados pessoais imediatos)
...
- **Dependent**: Tabela de relacionamento que não herda de Entity/Person
  - FK `dependent_patient_id`: PatientAccess do dependente (quem é dependente)
  - FK `guardian_patient_id`: PatientAccess do responsável (quem cuida)
...
- **PatientAccess**: Herda de Person (via Entity)
  ...
  - Cada PatientAccess pode ser tanto dependente quanto responsável
  
  **Nota arquitetural**: Dependentes precisam ser registrados como PatientAccess (não apenas Dependent) mesmo que não possam acessar o sistema por conta própria. Exemplo: um bebê é cadastrado como PatientAccess dependente de um responsável, com email e senha placeholder inicializados para permitir agendamentos de consultas. O email placeholder deve ser único no banco para preservar a constraint `unique` de `PatientAccess.email`, e o sistema deve exigir atualização por um email real no primeiro uso. A tabela Dependent não herda de Person porque é apenas um relacionamento de responsabilidade entre dois PatientAccess, não uma entidade de acesso em si.

### Regras de Negócio Implementadas
1. Um dependente possui exatamente um responsável
2. Um responsável pode ter zero ou mais dependentes
3. Dependente é sempre um PatientAccess (tem conta de paciente própria)
4. Responsável é sempre um PatientAccess
5. Um PatientAccess pode ser simultaneamente responsável de alguns e dependente de outro
...
- Dependentes possuem registro próprio no PatientAccess, mesmo que não acessem diretamente (ex: bebês agendados com email do responsável)
...
1. Dependente é registrado como PatientAccess com email e senha do responsável
   - Mesmo que seja um bebê ou incapaz de usar a conta, o registro no PatientAccess é obrigatório
   - Credenciais são inicializadas com dados do responsável (permite que responsável acesse em nome do dependente)
2. Se dados pessoais (Person) não estão disponíveis inicialmente, podem ser adicionados posteriormente
3. Na tabela Dependent, cria-se FK vinculando PatientAccess do dependente ao PatientAccess do responsável
